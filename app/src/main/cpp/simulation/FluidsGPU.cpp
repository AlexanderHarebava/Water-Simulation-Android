...
void main() {
    uint lid = gl_LocalInvocationIndex;
    float v = 0.0;
    for (int i = int(lid); i < uNumGroups; i += 128) v += sums[i];
    sdata[lid] = v;
    barrier();
    for (uint s = 64u; s > 0u; s >>= 1u) {
        if (lid < s) sdata[lid] += sdata[lid + s];
...
void FluidsGPU::init(int N, int width, int height) {
    ...
    if (maxInvocations > 0 && maxInvocations < 128) {
        __android_log_print(ANDROID_LOG_WARN, TAG_GPU,
                            "Device supports only %d invocations/group (< 128)! "
...
void FluidsGPU::dispatchExportDensity() {
    ...
    const int groups = (packedWords + 127) / 128;
