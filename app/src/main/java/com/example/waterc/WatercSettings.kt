    ...
    companion object {
        ...
        fun load(context: Context): WatercSettings {
            ...
            return WatercSettings(
                simSpeed = p.getFloat(KEY_SIM_SPEED, DEFAULT_SIM_SPEED).coerceIn(0.3f, 1.0f),                waterR = p.getFloat(KEY_WATER_R, DEFAULT_WATER_R).coerceIn(0f, 1f),
        ...
        fun reset(context: Context, synchronous: Boolean = true) {
            ...
            if (synchronous) editor.commit() else editor.apply()
            save(context, WatercSettings(), synchronous)
        }
